...
bool Adafruit_MLX90393::startBurstMode(uint8_t axis_mask = MLX90393_AXIS_ALL)
{
    uint8_t command = MLX90393_REG_SB
        | MLX90393_AXIS_ALL; // Send burst command for all axes
    uint16_t result;

    return writeRegister(command, 0x00);
}

bool Adafruit_MLX90393::readBurstData(
    float* x, float* y, float* z, uint8_t num_readings)
{
    for (uint8_t i = 0; i < num_readings; i++) {
        if (!readBurstMeasurement(x, y, z)) {
            return false; // Return false if any reading fails
        }
        // Delay or manage timing based on the sensor's sampling rate
        delay(10); // Adjust delay as needed
    }
    return true;
}

bool Adafruit_MLX90393::readBurstMeasurement(float* x, float* y, float* z)
{
    // Burst mode should already be active, just read the data
...
bool Adafruit_MLX90393::setResolution(
    enum mlx90393_axis axis, enum mlx90393_resolution resolution)
{
...
uint8_t Adafruit_MLX90393::transceive(uint8_t* txbuf, uint8_t txlen,
    uint8_t* rxbuf, uint8_t rxlen, uint8_t interdelay)
{
    uint8_t status = 0;
    uint8_t i;
